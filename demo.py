...
def demo():
    ...
    if records:
        ...
        records, summary, keys = execute_query(
            NON_EXISTENT_RELATION_QUERY,
            {"name_id": name_id, "title_id": non_existent_title_id},
        )

        if not summary.counters.relationships_created:
            print(
                f"No relationship created between {name} and Title with non existend id {non_existent_title_id}"
            )
        ...
        GET_NAME_TITLES_BETWEEN_YEARS = """
            ...
            RETURN t.id AS id, t.title AS title, t.start_year AS year;
        """
        records, summary, keys = execute_query(
            GET_NAME_TITLES_BETWEEN_YEARS,
            {"name_id": name_id, "start_year": year1, "start_year_2": year2},
        )

        if not records:
            print(
                f"{name} has not acted in Titles that were initiated between {year1} and {year2}."
            )
            return
        ...
        for i in range(0, 2):
            records, summary, keys = execute_query(
                TITLE_DIRECTED_BY, {"title_id": title_id, "name_id": name_id}
            )
